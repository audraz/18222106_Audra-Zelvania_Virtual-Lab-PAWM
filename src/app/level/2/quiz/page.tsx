...
const QuizPage = () => {
  ...
  const saveQuizProgressToLocal = (userId: string) => {
    ...
    localStorage.setItem(`quiz_progress_level_2_${userId}`, JSON.stringify(progressData));
  };
  
  const getQuizProgressFromLocal = (userId: string) => {
    const data = localStorage.getItem(`quiz_progress_level_2_${userId}`);
  ...
  const handleFinishQuiz = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        userProgress.level_2 = true;
        userProgress.level_3 = true;
  ...
  const resetQuiz = () => {
    ...
    const user = auth.currentUser;
    if (user) localStorage.removeItem(`quiz_progress_level_2_${user.uid}`);
